...
use redis_rs::server::{Config, Server};
use redis_rs::stats::{Stats, StatsSnapshot};
...
fn ui(frame: &mut ratatui::Frame, snap: &StatsSnapshot) {
    ...
    let title_text = Line::from(vec![
        Span::styled(" redis-rs Monitor ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::raw("  "),
        Span::styled(&snap.addr, Style::default().fg(Color::Yellow)),
        Span::raw("  "),
        Span::styled(format_uptime(snap.uptime_secs), Style::default().fg(Color::Green)),
    ]);
    ...
    let left_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Length(7), Constraint::Length(4), Constraint::Min(0)])
        .split(body_chunks[0]);
    ...
    let keys_info = Paragraph::new(vec![
        Line::from(format!(" Stored keys: {}", "? (query via KEYS)")),
    ])
    ...
    let cmds_list = List::new(cmds)
        .block(Block::default().title(" Recent Commands ").borders(Borders::ALL));
